...
namespace witch_alexa_skill.Utils.Id
{
    public static class ShortId
    {
        ...
        public static string Generate(GenerationOptions options)
        {
            ...
            if (!options.UseAlphabet &&
                !options.UseAlphabetBig &&
                !options.UseAlphabetSmall &&
                !options.UseNumbers &&
                !options.UseSpecialCharacters)
            {
                throw new ArgumentException("You have not selected any chars to use");
            }
            ...
            if (options.UseAlphabet)
            {
                poolBuilder.Append($"{Bigs}{Smalls}");
            }       

            if (options.UseAlphabetBig)
            {
                poolBuilder.Append(Bigs);
            }

            if (options.UseAlphabetSmall)
            {
                poolBuilder.Append(Smalls);
            }
